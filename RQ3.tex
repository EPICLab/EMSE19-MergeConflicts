...
We framed the \textit{Barriers Survey}~(S2) questions to validate the improvement needs expressed in our interviews, and ranked those six needs according to mean score.
...
In addition, we also asked \textit{Barriers Survey}~(S2) participants which tools they use during conflict resolution.
...
\toprule
  ...
  Survey participants were allowed to provide multiple tools. Each entry represents the number (and percentage) of participants that responded with that particular tool. 115 out of 162 participants (70.99\%) indicated the use of at least one merge resolution tool.\vspace*{-0.3\baselineskip}}\\
\midrule
...
Our \textit{Barriers Survey}~(S2) results indicate that \textit{better usability} (I1) is the most desired improvement of toolsets used for conflict resolution. 
...
\begin{figure}
	...
	\caption{Degree of Trust for Merging, History Exploration, and Conflict Resolution Tools. Scale: 1 is \textit{completely} and 5 is \textit{not at all}. 121 out of 162 participants (74.69\%) provided a response to this question in the \textit{Barriers Survey}~(S2).\vspace*{-0.3\baselineskip}}
	\label{fig:tool_trust}
\end{figure}

Based upon this theme of mistrust, we asked \textit{Barriers Survey}~(S2) participants to rate the degree to which they trust their merging, history exploration, and conflict resolution tools.
...
\caption{Effectiveness of developers' toolsets in supporting perceived size and complexity of merge conflicts. Dot values indicate the number of \textit{Barriers Survey}~(S2) responses for effectiveness of a particular merge conflict size and complexity, and the vertical height of each segment indicating the number of responses at each degree of effectiveness.\vspace*{-0.3\baselineskip}}
